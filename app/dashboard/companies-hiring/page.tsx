...
function CompaniesHiringContent() {
  ...
  const handleArchiveCompany = async (id: string) => {
    ...
    try {
      await fetch(`/api/companies/${id}`, { method: 'DELETE', body: JSON.stringify({ archiveReason: 'Archived by user' }) });
      fetchData();
    } catch (e) {}
  };
  ...
  const handleRestoreCompany = async (id: string) => {
    try {
      await fetch(`/api/companies/${id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'restore' }) });
      fetchData();
    } catch (e) {}
  };
  ...
  const handleBulkArchive = async () => {
    ...
    try {
      ...
      await fetch(`/api/companies/bulk-archive`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyIds: Array.from(selectedIds) }) });
  ...
  const handleBulkRestore = async () => {
    try {
      ...
      await fetch(`/api/companies/bulk-restore`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ companyIds: Array.from(selectedIds) }) });
      setSelectedIds(new Set());
      fetchData();
    } catch (e) {} finally { setActionProgress(null); }
  };
  ...
  const handleAddToLead = async (id: string) => {
    try {
      ...
      const res = await fetch(`/api/companies/${id}/add-to-leads`, { method: 'POST' });
      const data = await res.json();
  ...
  const handleAddToCampaign = async (campaignId: string) => {
    ...
    for (const id of idsToProcess) {
      try {
        const res = await fetch(`/api/companies/${id}/add-to-campaign`, {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate-400">Track real hiring activity and recruitment demand by company.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <div className="glass p-4 rounded-xl border border-slate-800">
          <p className="text-xs text-slate-400 mb-1">Total Active Job Posts Found</p>
          <p className="text-2xl font-bold text-white">{isLoading ? '-' : summary.totalActiveJobPosts || 0}</p>
        </div>
        <div className="glass p-4 rounded-xl border border-slate-800">
          <p className="text-xs text-slate-400 mb-1">Total Categories Found</p>
          <p className="text-2xl font-bold text-white">{isLoading ? '-' : summary.totalCategories || 0}</p>
        </div>
        <div className="glass p-4 rounded-xl border border-slate-800">
          <p className="text-xs text-slate-400 mb-1">Total Locations Found</p>
          <p className="text-2xl font-bold text-white">{isLoading ? '-' : summary.totalLocations || 0}</p>
        </div>
        <div className="glass p-4 rounded-xl border border-slate-800">
          <p className="text-xs text-slate-400 mb-1">Exact Vacancies Publicly Disclosed</p>
          <p className="text-2xl font-bold text-white">{isLoading ? '-' : summary.exactVacanciesDisclosed || 0}</p>
        </div>
        <div className="glass p-4 rounded-xl border border-slate-800">
          <p className="text-xs text-slate-400 mb-1">High Hiring Activity Companies</p>
          <p className="text-2xl font-bold text-white">{isLoading ? '-' : summary.highActivityCompanies || 0}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between glass p-4 rounded-xl border border-slate-800">
        ...
        <div className="flex gap-2 w-full md:w-auto overflow-x-auto">
          ...
          <select className="bg-slate-800 border border-slate-700 text-white text-sm rounded-lg px-3 py-2" value={sort} onChange={(e) => updateUrlParams({ sort: e.target.value, page: '1' })}>
            <option value="most-active">Most Active Hiring</option>
            <option value="most-recent">Most Recent Posting</option>
            <option value="most-jobs">Most Job Posts</option>
            ...
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-slate-900/80 border-b border-slate-800 text-slate-400 text-xs uppercase">
                  ...
                  <th className="p-4 font-medium">Active Posts</th>
                  ...
                  <th className="p-4 font-medium">Exact Vacancies</th>
                ...
                {companies.map(c => (
                  <tr key={c.id} className={`hover:bg-slate-800/50 transition-colors ${c.archived ? 'opacity-50' : ''}`}>
                    ...
                    <td className="p-4">
                      ...
                      {c.archived && <span className="text-[10px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded mr-2">Archived</span>}
                      ...
                      {c.addedToCampaign && <span className="text-[10px] bg-purple-900/30 text-purple-400 px-1.5 py-0.5 rounded border border-purple-900 ml-1">Campaign</span>}
                    </td>
                    <td className="p-4"><span className="font-mono bg-slate-900 px-2 py-1 rounded text-purple-400 border border-slate-700">{c.activeJobPostsFound}</span></td>
                    <td className="p-4 text-slate-400 max-w-xs truncate" title={c.jobCategories.join(', ')}>{c.jobCategories.join(', ') || '-'}</td>
                    <td className="p-4 text-slate-400 max-w-[150px] truncate" title={c.locations.join(', ')}>{c.locations.join(', ') || '-'}</td>
                    <td className="p-4"><span className={`text-xs px-2 py-1 rounded-full ${c.hiringActivity === 'High Activity' ? 'bg-green-500/20 text-green-400' : c.hiringActivity === 'Medium Activity' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-slate-800 text-slate-400'}`}>{c.hiringActivity}</span></td>
                    <td className="p-4 text-slate-300">{c.exactVacanciesDisclosed > 0 ? <span className="font-bold">{c.exactVacanciesDisclosed}</span> : '-'}</td>
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => handleAddToLead(c.id)} className="p-1.5 text-slate-400 hover:text-green-400 bg-slate-800 rounded" title="Add to Leads"><UserPlus size={14}/></button>
                        <Link href={`/dashboard/companies-hiring/${c.companyId}`} className="p-1.5 text-slate-400 hover:text-white bg-slate-800 rounded" title="View Details"><Eye size={14}/></Link>
                      </div>
                    </td>
                  </tr>
                ))}
              ...
              <div key={company.id} className={`glass p-6 rounded-2xl border ${selectedIds.has(company.id) ? 'border-purple-500 bg-purple-500/5' : 'border-slate-800'} hover:border-purple-500/50 transition-all flex flex-col h-full relative ${company.archived ? 'opacity-70 grayscale-[50%]' : ''}`}>
                ...
                <div className="flex justify-between items-start mb-4 pl-8">
                  ...
                  <div className="flex gap-2 relative group cursor-pointer shrink-0 ml-2">
                    ...
                    <div className="absolute right-0 top-6 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 overflow-hidden">
                       <Link href={`/dashboard/companies-hiring/${company.companyId}`} className="block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700 hover:text-white"><Eye size={14} className="inline mr-2"/> View Details</Link>
                       ...
                       {!company.archived ? (
                         ...
                         <button onClick={() => handleRestoreCompany(company.id)} className="w-full text-left px-4 py-2 text-sm text-green-400 hover:bg-slate-700">Restore</button>
                       )}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-800/80">
                    <p className="text-[10px] uppercase text-slate-500 font-bold mb-1">Active Posts</p>
                    <p className="text-2xl font-black text-white">{company.activeJobPostsFound}</p>
                    <p className="text-[10px] text-slate-400 mt-1">+{company.recentPosts7Days} in 7 days</p>
                  </div>
                  <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-800/80 flex flex-col justify-between">
                    <div>
                      <p className="text-[10px] uppercase text-slate-500 font-bold mb-1">Activity</p>
                      <p className={`text-sm font-bold ${company.hiringActivity === 'High Activity' ? 'text-green-400' : company.hiringActivity === 'Medium Activity' ? 'text-yellow-400' : 'text-slate-400'}`}>{company.hiringActivity}</p>
                    </div>
                    {company.exactVacanciesDisclosed > 0 && (
                      <div className="mt-2 pt-2 border-t border-slate-800/50">
                        <p className="text-[10px] uppercase text-purple-400 font-bold">Vacancies: {company.exactVacanciesDisclosed}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-2 mb-6 flex-grow">
                  <div className="flex items-start gap-2 text-sm text-slate-300">
                    ...
                    <span className="line-clamp-2">{company.jobCategories.slice(0, 3).join(', ') || 'Various roles'} {company.jobCategories.length > 3 && `+${company.jobCategories.length - 3} more`}</span>
                  </div>
                  <div className="flex items-start gap-2 text-sm text-slate-300">
                    <MapPin size={16} className="text-slate-500 mt-0.5 shrink-0" />
                    <span className="line-clamp-2">{company.locations.slice(0, 3).join(', ') || 'Multiple locations'} {company.locations.length > 3 && `+${company.locations.length - 3} more`}</span>
                  </div>
                </div>
  
                <div className="flex gap-2 mt-auto pt-4 border-t border-slate-800">
                  <button onClick={() => handleAddToLead(company.id)} className="flex-1 bg-purple-600/20 hover:bg-purple-600/40 text-purple-300 border border-purple-500/30 text-center py-2 px-4 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
                    {company.addedToLeads ? 'Update Lead' : 'Add Lead'}
                  </button>
                  <button onClick={() => handleOpenCampaignModal(company.id)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white text-center py-2 px-4 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
                    Campaign
                  </button>
                </div>
              </div>
